...
public interface PostRepository extends JpaRepository<Post, Long> {
    ...
    @Query("SELECT CASE WHEN p.userId = :userId THEN 1 ELSE 0 END AS isOwnedInt, p.userId AS userId, " +
            ...
            "CONCAT(e.type, '') AS type, e.calo, CONCAT(r.step, ' steps') AS score, " +
    ...
    @Query("SELECT CASE WHEN p.userId = :userId THEN 1 ELSE 0 END AS isOwnedInt, p.userId AS userId, " +
            "p.id AS postId, u.name AS username, p.title AS title, p.createdAt AS createdAt, " +
            "CONCAT(e.type, '') AS type, e.calo, CONCAT(b.distance, ' meters') AS score, " +
    ...
    @Query("SELECT CASE WHEN p.userId = :userId THEN 1 ELSE 0 END AS isOwnedInt, p.userId AS userId, " +
            "p.id AS postId, u.name AS username, p.title AS title, p.createdAt AS createdAt, " +
            "CONCAT(e.type, '') AS type, e.calo, CONCAT(pu.rep, ' reps') AS score, " +
    ...
    @Query("SELECT CASE WHEN p.userId = :userId THEN 1 ELSE 0 END AS isOwnedInt, p.userId AS userId, " +
            "p.id AS postId, u.name AS username, p.title AS title, p.createdAt AS createdAt, " +
            "CONCAT(e.type, '') AS type, e.calo, CONCAT(COUNT(we.id), ' reps') AS score, " +
